 ...
 *
 * Derived from englmaxi/zmk-dongle-display (MIT). Animation/rendering kept
 * as-is; only the data transport changed to the Prospector BLE scanner (WPM
 * from the advertisement drives the animation). See ATTRIBUTION.md.
 */
...
void zmk_widget_bongo_cat_set(uint8_t wpm) {
    bongo_cat_wpm_status_update_cb((struct bongo_cat_wpm_status_state){.wpm = wpm});
}
